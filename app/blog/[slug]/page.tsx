import PostContent from "./components/PostContent";
import { PostList, PostProps } from "./components/types";

/* function isPost(object: any): object is PostList {
    return object;
} */

const getPost = async (slug: string) => {

    const res = await fetch(`https://landblog-production.up.railway.app/api/posts?filters[slug][$contains]=${slug}`, {
        headers: {
            "Authorization": "bearer "
        }
    })
    const data = await res.json();
    return(data.data[0])
}

export default async function Page({ params }: PostProps) {
    const { slug } = params;

    const post = await getPost(slug)

    if (post) {
        return <PostContent post={post} />
    }
    return <div>Nope</div>
}

export async function generateStaticParams() {
    const res = await fetch(`https://landblog-production.up.railway.app/api/posts`, {
        headers: {
            "Authorization": "bearer "
        }
    })

    const data = await res.json()
    const postSlugs: PostList[] = data.data

    return postSlugs.map(post => ({
        slug: post.attributes.Slug
    }))
  }

export const dynamicParams = false